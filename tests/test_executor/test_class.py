...
#
# NOTE 2: this/__class__는 이 파일에서 return값으로 캡처해 검증한다.
# (전역 변수 대입은 이제 호출이 끝난 뒤에도 유지되므로(_storage.py의
# push_call_frame) 전역 캡처로도 검증할 수 있지만, this/__class__ 값
# 자체는 애초에 전역이 아니라 호출별 지역 바인딩이라 return이 더 직접적이다.)
