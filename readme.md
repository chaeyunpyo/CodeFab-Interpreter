...
| --- | --- |
| 팀명 | ErrorZero |
| 팀 소개 | 코드 리뷰를 통해 오류를 사전에 예방하고, 더 높은 품질의 코드를 만들어가는 팀 |
| 팀장 | 채윤표 |
| 팀원 | 이용진, 김문정, 강보경, 신경섭 |
...
## 추가 기능 체크리스트
(상세 근거는 `요구사항_정리/개발_현황.md` 참고).

| 기능 | 설명 | 상태 |
| --- | --- | --- |
| 함수 | 선언/호출/return/재귀 | ✅ 완료 |
| 클래스 | 필드/메서드/생성자/상속/super/instanceof | ✅ 완료 |
| 정적 배열 | 고정 크기 배열 생성/읽기/쓰기 | ✅ 완료 |
| 실행 전 최적화 | 정적 바인딩, 상수 연산 최적화 | ✅ 완료 |
| import | Library import, alias, 순환/중복 import 오류 검사 | ✅ 완료 |
| 공장제어쉘 | 프롬프트 모드 / 파일 모드 / 디버그 모드 | ✅ 완료 |
...
- 함수는 클로저를 지원하지 않는다. 함수 호출마다 호출부의 지역 변수 체인은 초기화되고, 전역 스코프만 그대로 유지된다 (`src/executor/_function.py` 참고).
- `instanceof`는 자기 자신뿐 아니라 상속 체인상의 조상 클래스에 대해서도 `true`를 반환한다.
- 전역 변수는 함수 안에서 대입해도 함수가 끝난 뒤까지 값이 유지된다 (진짜 "전역"이므로 `return` 없이도 반영됨). 다만 함수 안에서 지역 변수(파라미터/블록 변수)로 같은 이름을 새로 선언하면 그림자가 생겨, 그 지역 변수에 대입해도 전역 값은 바뀌지 않는다.
- `Array`처럼 내장으로 제공되는 이름은 전역 스코프에서 재선언(`var Array = ...;`)하거나 대입(`Array = ...;`)할 수 없다 - Checker가 정적으로 막는다. 지역 스코프에서 같은 이름으로 그림자를 만드는 건 일반 변수 shadowing과 동일하게 허용된다.
